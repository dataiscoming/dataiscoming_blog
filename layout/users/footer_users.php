<!-- Footer
================================================== -->
<footer>

    <div class="row">       

        <div class="six columns tab-whole footer-about">
				
			<h3>About Data is coming</h3>
               
            <p>A blog about Game Of Thrones and data science</p>      

        </div> <!-- /footer-about -->

        <div class="six columns tab-whole right-cols">

			<div class="row">

				<div class="columns">
					<h3 class="address">Contact me</h3>

					<ul>
						<li><a href="mailto:quentin.mouton@laposte.net">quentin.mouton@laposte.net</a></li>
					</ul>                  
				</div> <!-- /columns -->             

				<div class="columns last">
					<h3 class="contact">Follow me</h3>

					<ul>
						<li onclick="SendEvent(LINK='https://github.com/dataiscoming',ELEMENT='footer_github')"><a href="https://github.com/dataiscoming" target="_blank">Github</a></li>
						<li><a href="../admin/login/login_admin_form.php" target="_blank">Administrator</a></li>
					</ul>
                  
				</div> <!-- /columns --> 

            </div> <!-- /Row(nested) -->

        </div>

        <p class="copyright">&copy; Copyright 2014 Puremedia. Design by Styleshout.</p>        

	</div> <!-- /row -->
	  
</footer> <!-- /footer -->